...
library(dplyr)
library(plyr)
library(ggplot2)  #### Added in 4th commit
...
#### Unused code commented out in third commit ####
####  Occupancy Summaries  ##
...
#### Following section expanded with SD in 4th commit ####
...
PDvsLA.data0 <- cbind(LA.data[,1:3],PD.data[,2:3])
PDvsLA.data <- PDvsLA.data0[c(-11,-13),]
colnames(PDvsLA.data) <- c("Type", "Mean(LA)", "SD(LA)",
                           "Mean(PD)", "SD(PD)")

qplot(factor(Type), data=PDvsLA.data, geom="bar",
      fill=c("Mean(LA)","Mean(PD)"), weight=, position="dodge",
      main="Mean Travel Time by Type")
